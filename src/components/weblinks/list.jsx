...
const ENDPOINT = 'weblinks'

class WeblinkListComponent extends Component {
    state = {
        ...
        columns: [
            ...
            },
            {
                ...
                Header: 'Publié',
                width:100,
                Cell: ({ row: { _original } }) => <Toggle checked={_original.public==1}  onChange={() => this.weblinkUpdatePublic(_original)} />,
    ...
    renderActions = (row) => (
        <>
            <Button size="sm" outline color="info" onClick={() => this.props.setCurrentWeblink(row)}><i className="fa fa-edit"></i></Button> {"   "}
            <Button size="sm" outline color="danger" onClick={() => this.removeWeblink(row)}><i className="fa fa-trash"></i></Button>
        </>
    );
...
const mapStateProps = (state) => {
    return {
        ...
        reload_data: state.weblink.reload_data,
    }
}
...
const mapDispatchToProps = dispatch => {
    ...
    return {
        setCurrentWeblink: (current) => {
            dispatch({ type: 'current-weblink', current })
        },
        reloadDataAfterEvent: (event) => dispatch({ type: 'reload-data', event }),
    }
}

export default connect(mapStateProps, mapDispatchToProps)(WeblinkListComponent)
